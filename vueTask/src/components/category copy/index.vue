...
<template>
 ...
 <div class="row">
  <RouterLink to="/storeBrand" class="btn btn-primary add">Add Brand </RouterLink>
   
 </div>
<br>
   <input v-if="brand.length>0" type="text" v-model="searchTerm" class="form-control" style="width: 300px;" placeholder="Search Here">

<h3 class="text-center" v-if="brand.length == 0">no brand added </h3>
<br>

   <div class="row"  v-if="brand.length>0">
...
import { API } from '../../config/config'
  export default {
    ...
    computed:{
      filtersearch(){
      return this.brand.filter(brand => {
         ...
         this.$swal({
             ...
             this.$swal.fire({
              ...
              if (result.value) {
               ...
               .then(() => {
                this.brand = this.brand.filter(brand => {
                  return brand.id != id
                })
               })
               .catch(() => {
